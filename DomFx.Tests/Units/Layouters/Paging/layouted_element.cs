...
{
    public class layouted_element
    {
        [Fact]
        public void forced_outerheight_is_transferred_to_fixed_element()
        {
            var element = new LayoutedElement(new Box(), Children.Empty) { ForcedOuterHeight = 2.cm() };
            var page = Page.First();
        ...
        {
            var element = new LayoutedElement(new Box(), Children.Empty) { ForcedInnerWidth = 2.cm() };
            var page = Page.First();
            page.Add(new Line(0.cm(), false, element), 0.cm());
